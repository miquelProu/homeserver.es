...
if ( ! class_exists( 'Understrap_WP_Bootstrap_Navwalker' ) ) {
	...
	class Understrap_WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
            ...
            if ( end( $matches[2] ) ) {
               array_push($classes, end( $matches[2] ));
            }
			...
			$output .= "{$n}{$indent}<ul$class_names $labelledby depth=\"".$depth."\" role=\"menu\">{$n}";
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		    ...
		    if ($item->title == 'Menu Gran') {
                $posst= get_post(apply_filters('wpml_object_id', 62, 'post', true));
                $output .= "<div class='menu-gran d-md-block d-none'>".$posst->post_content."</div>";
            } elseif ($item->title == 'menu otros') {
                $posst= get_post(apply_filters('wpml_object_id', 439, 'post', true));
                $output .= "<div class='menu-gran d-md-block d-none'>".$posst->post_content."</div>";
            }
		    else {
